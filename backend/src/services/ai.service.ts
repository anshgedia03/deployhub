...
export const createLangChainTools = (userId: string, organizationId: string, res: Response) => {
  const getOrganizationEmployeesTool = new DynamicStructuredTool({
    ...
    schema: z.object({
      queryPurpose: z.string().optional().describe('Brief reason for fetching employees'),
    }),
    func: async () => {
  ...
  const getUserDeploymentsTool = new DynamicStructuredTool({
    ...
    schema: z.object({
      searchFilter: z.string().optional().describe('Optional filter for project name or status'),
    }),
  ...
  const getContainerHealthTool = new DynamicStructuredTool({
    ...
    schema: z.object({
      projectName: z.string().optional().describe('Optional specific project container name'),
    }),
...
export const processAIQuery = async (
  ...
  if (!apiKey) {
    ...
    if (lowerQuery.includes('employee') || lowerQuery.includes('user') || lowerQuery.includes('team') || lowerQuery.includes('people') || lowerQuery.includes('organization')) {
      ...
      const reply = `### Organization Employees Summary\n\nYour organization currently has **${empData.totalEmployees}** total member(s):\n\n` +
        empData.employees.map((e: any) => `- **${e.username}** (\`${e.email}\`) — Role: *${e.role}* (${e.accountType})`).join('\n') +
    ...
    if (lowerQuery.includes('deploy') || lowerQuery.includes('project') || lowerQuery.includes('app') || lowerQuery.includes('status')) {
      ...
      const reply = `### Organization Deployments Summary\n\nYour organization has **${depData.totalDeployments}** project deployment(s):\n\n` +
        depData.deployments.map((d: any) => `- **${d.projectName}**: Status \`${d.status}\`${d.publicUrl ? ` — [Live App](${d.publicUrl})` : ''} (Port: ${d.port || 'N/A'})`).join('\n') +
        `\n\n*Note: Configure \`GROQ_API_KEY\` in environment for advanced multi-tool LLM reasoning.*`;
  ...
  try {
    ...
    const messages: BaseMessage[] = [
      new SystemMessage(
        'You are DeployHub AI, an expert cloud infrastructure & DevOps AI agent. ' +
